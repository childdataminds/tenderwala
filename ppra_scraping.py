...
class Faderal_Scraper:
    ...
    def initiate_scraper(self):
        self.ppra_data = []  
        c = 1
        print("Scraping Federal PPRA Tenders... at time: ", self.utils.get_datetime())
        while True:
            # print("Page: ",str(c))
    ...
    def inner_ppra(self,page_no):
        ...
        try:
            resp = requests.get(ppra_url, headers=self.ppra_header, timeout=30)
        ...
        if resp.status_code == 200:
            ...
            if "no active tenders found" in page_text:
                return [False]
            ...
            if table:
                ...
                for row in rows:
                    try:
                        ...
                        opening_strong = cols[6].find("strong")
                        if opening_strong is None:
                            continue

                        # Opening Date
                        opening_date = opening_strong.get_text(strip=True)
                        ...
                        # Tender No
                        tender_no_tag = row.select_one(".tender-no strong")
                        if tender_no_tag is None:
                            continue
                        tender_no = tender_no_tag.get_text(strip=True)

                        # Title
                        title_tag = cols[2].find("strong")
                        ...
                        # City
                        city_list = cols[3].select("small")
                        city = ""
                        if len(city_list) > 0:
                            city = city_list[-1].get_text(strip=True).split("-")[0].strip()
        ...
        return [False, f"Federal: Unexpected HTTP status {resp.status_code} on page {page_no}"]
