...
class Utilities:
    ...
    def update_texted_on(self,phone,time):
        ...
        resp = db_execute(payload)
        return resp
    def update_window_reminder_sent_on(self,phone,time):
        payload =  {
            "db":"tenderwala",
            "table":"users_table",
            "cols":["window_reminder_sent_on"],
            "ops":"UPDATE",
            "where":["phone"],
            "value":[time,phone]
        }
    ...
    def update_user_status(self,phone,status):
         ...
         if status_norm == "TRIAL":
            ...
            payload =  {
               "db":"tenderwala",
               "table":"users_table",
               "cols":["status","subs_date"],
               "ops":"UPDATE",
               "where":["phone"],
               "value":[status_norm,sub_date,phone]
            }
         else:
            payload =  {
               "db":"tenderwala",
    ...
    def insert_into_filters(self,phone,col,data,available=False):
        row_exists = False
        if available:
           existing = self.get_filters(phone)
           row_exists = bool(existing[0])

        if available and row_exists:
           where = ["phone"]
           value = [data,phone]
           cols = [col]
           ops = "UPDATE"
        ...
        resp = db_execute(payload)
        return resp
    def reset_city_filters(self, phone):
        payload = {
            "db": "tenderwala",
            "table": "filters_table",
            "cols": [
            ...
            "ops": "UPDATE",
            "where": ["phone"],
            "value": ["empty", "empty", "empty", "empty", "empty", "empty", phone]
        }
        resp = db_execute(payload)
        return resp
    def save_filters(self, phone, filter_values):
        ...
        merged_values = {col: "empty" for col in filter_cols}
        if isinstance(filter_values, dict):
            for col in filter_cols:
                if col in filter_values:
                    raw_value = str(filter_values[col]).strip()
                    merged_values[col] = raw_value if raw_value != "" else "empty"

        existing = self.get_filters(phone_key)
        if existing[0]:
            payload = {
                "db": "tenderwala",
                "table": "filters_table",
                ...
                "ops": "UPDATE",
                "where": ["phone"],
                "value": [merged_values[col] for col in filter_cols] + [phone_key]
            }
        else:
            payload = {
                "db": "tenderwala",
                "table": "filters_table",
                "cols": ["phone"] + filter_cols,
                "ops": "INSERT",
                "where": None,
                "value": [phone_key] + [merged_values[col] for col in filter_cols]
            }
